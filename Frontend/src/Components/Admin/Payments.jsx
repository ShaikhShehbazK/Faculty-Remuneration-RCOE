...
function Payments() {
  ...
  const handleFacultyChange = async (facultyId) => {
    ...
    if (facultyId) {
      try {
        setLoading(true);
        // Fetch faculty data
        const facultyResponse = await axios.get(`http://localhost:3002/admin/payment/faculty/${facultyId}`);
        setFacultyData(facultyResponse.data);

        // Fetch semesters for this faculty
        const semestersResponse = await axios.get(`http://localhost:3002/admin/payment/faculty/${facultyId}/semesters`);
  ...
  const handleSemesterChange = async (semester) => {
    ...
    if (semester && selectedFaculty) {
      try {
        setLoading(true);
        const response = await axios.get(`http://localhost:3002/admin/payment/faculty/${selectedFaculty}/semester/${semester}/subjects`);
  ...
  const handleSubjectChange = async (subjectName) => {
    ...
    if (subjectName) {
      try {
        ...
        const selectedSubjectObj = subjects.find(subject => subject.name === subjectName);
  ...
  const handleSavePayment = async () => {
    ...
    try {
      ...
      console.log('Available subjects:', subjects);
       ...
       const termWorkTotal = selectedSubjectDetails?.hasTermTest ? (parseInt(termWorkPapers) || 0) * (parseInt(termWorkRate) || 0) : 0;
       const oralTotal = selectedSubjectDetails?.hasPractical ? (parseInt(oralPapers) || 0) * (parseInt(oralRate) || 0) : 0;
       const semesterTotal = selectedSubjectDetails?.hasSemesterExam ? (parseInt(semesterPapers) || 0) * (parseInt(semesterRate) || 0) : 0;
  ...
  const handleUpdateFinalCalculation = async () => {
    ...
    try {
      setLoading(true);
       ...
       const subjectBreakdown = calculatedRemunerations.map(remuneration => {
         ...
         if (!subjectObj) {
           subjectObj = subjects.find(subject => 
             subject.name.toLowerCase() === remuneration.subjectName.toLowerCase()
           );
         }
         ...
         if (!subjectObj) {
           subjectObj = subjects.find(subject => 
             subject.name.toLowerCase().includes(remuneration.subjectName.toLowerCase()) ||
             remuneration.subjectName.toLowerCase().includes(subject.name.toLowerCase())
           );
         }
         ...
         console.log(`Matched "${remuneration.subjectName}" with "${subjectObj.name}"`);
       ...
       const uniqueSubjectBreakdown = subjectBreakdown.filter((item, index, self) => 
              ...
              <Col md={4}>
                <Form.Group>
                  ...
                  <InputGroup>
                    ...
                    <Form.Control 
                      type="text" 
                      value={facultyData ? facultyData.baseSalary?.toLocaleString() : "0"} 
                      disabled 
                    />
                  </InputGroup>
                </Form.Group>
              ...
              <Col md={4}>
                <Form.Group>
                  ...
                  <InputGroup>
                    ...
                    <Form.Control 
                      type="text" 
                      value={facultyData ? facultyData.travelAllowance?.toLocaleString() : "0"} 
                      disabled 
                    />
                  </InputGroup>
                </Form.Group>
                 ...
                 {selectedSubjectDetails && (
                   <div className="mt-2">
                     <span className="badge bg-info me-2">
                       {selectedSubjectDetails.hasTermTest ? "✓ Term Work" : "✗ Term Work"}
                     </span>
                     <span className="badge bg-info me-2">
                       {selectedSubjectDetails.hasPractical ? "✓ Practical" : "✗ Practical"}
                     </span>
                     <span className="badge bg-info">
                       {selectedSubjectDetails.hasSemesterExam ? "✓ Semester Exam" : "✗ Semester Exam"}
                     </span>
                   </div>
                 )}
               </small>
            ...
            <small className="text-muted mb-3 d-block">
             ...
             >
               <thead className="table-light">
                 <tr>
                   <th>Subject</th>
                   {calculatedRemunerations.some(r => r.termWorkPapers > 0) && <th>Term Work Papers</th>}
                   {calculatedRemunerations.some(r => r.oralPapers > 0) && <th>Oral/Practical Papers</th>}
                   {calculatedRemunerations.some(r => r.semesterPapers > 0) && <th>Semester Papers</th>}
                   <th>Semester</th>
                   <th>Total Payment</th>
                 </tr>
               </thead>
                 ...
                 {calculatedRemunerations.length === 0 ? (
                   <tr>
                     <td colSpan="6" className="text-center text-muted">
                       No remunerations calculated yet. Click "Add Subject to Calculation" to add subjects.
                     </td>
                   </tr>
                 ) : (
                   calculatedRemunerations.map((remuneration, index) => (
                     <tr key={index}>
                       ...
                       {calculatedRemunerations.some(r => r.termWorkPapers > 0) && (
                         <td>
                           {remuneration.termWorkPapers > 0 ? (
                             <>
                               {remuneration.termWorkPapers} <span className="badge bg-secondary ms-1">₹{remuneration.termWorkRate}</span>
                             </>
                           ) : (
                       ...
                       {calculatedRemunerations.some(r => r.oralPapers > 0) && (
                         <td>
                           {remuneration.oralPapers > 0 ? (
                             <>
                               {remuneration.oralPapers} <span className="badge bg-secondary ms-1">₹{remuneration.oralRate}</span>
                             </>
                           ) : (
                             <span className="text-muted">-</span>
                           )}
                       ...
                       {calculatedRemunerations.some(r => r.semesterPapers > 0) && (
                         <td>
                           {remuneration.semesterPapers > 0 ? (
                             <>
                               {remuneration.semesterPapers} <span className="badge bg-secondary ms-1">₹{remuneration.semesterRate}</span>
                             </>
                           ) : (
                             <span className="text-muted">-</span>
                           )}
                       ...
                       <td className="fw-bold text-success">₹{remuneration.totalPayment.toLocaleString()}</td>
                     </tr>
                   ))
                 )}
               </tbody>
                ...
                <tr>
                  ...
                  <td>
                    <span className="fw-bold text-primary">
                      ₹{facultyData ? facultyData.travelAllowance?.toLocaleString() : "0"}
                    </span>
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    <span className="fw-bold text-primary">
                      ₹{facultyData ? facultyData.baseSalary?.toLocaleString() : "0"}
                    </span>
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    <span className="fw-bold text-success">₹{calculatedRemunerations.reduce((sum, item) => sum + item.totalPayment, 0).toLocaleString()}</span>
                  </td>
                </tr>
                <tr className="fw-bold table-success">
                  <td>Total Remuneration</td>
                  <td>₹{(calculatedRemunerations.reduce((sum, item) => sum + item.totalPayment, 0) + (facultyData ? facultyData.baseSalary || 0 : 0) + (facultyData ? facultyData.travelAllowance || 0 : 0)).toLocaleString()}</td>
                </tr>
              </tbody>
            </Table>
